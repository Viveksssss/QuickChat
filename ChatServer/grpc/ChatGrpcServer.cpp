...
Status ChatGrpcServer::NotifyAddFriend(grpc::ServerContext *context,
...
Status ChatGrpcServer::NotifyAuthFriend(grpc::ServerContext *context,
                                        const AuthFriendRequest *request,
                                        AuthFriendResponse *response) {
    return Status::OK;
}
Status
ChatGrpcServer::NotifyTextChatMessage(grpc::ServerContext *context,
                                      const TextChatMessageRequest *request,
                                      TextChatMessageResponse *response) {
    auto to_uid = request->touid();
    auto session = UserManager::GetInstance()->GetSession(to_uid);
    Defer defer([request, response]() {
        response->set_error(static_cast<int>(ErrorCodes::SUCCESS));
    ...
    session->Send(request->data(),
                  static_cast<int>(MsgId::ID_NOTIFY_TEXT_CHAT_MSG_REQ));
    return Status::OK;
}

bool ChatGrpcServer::ChatGrpcServer::GetBaseInfo(
    std::string base_key, int uid, std::shared_ptr<UserInfo> &userinfo) {
    return true;
}

Status
ChatGrpcServer::NotifyMakeFriends(grpc::ServerContext *context,
                                  const NotifyMakeFriendsRequest *request,
                                  NotifyMakeFriendsResponse *response) {

    SPDLOG_INFO("Make Friends From {} to {}", request->fromuid(),
                request->touid());
    // 首先在本服务器查询
    auto to_uid = request->touid();
    auto session = UserManager::GetInstance()->GetSession(to_uid);
    ...
    j["from_email"] = request->fromemail();
...
Status
ChatGrpcServer::NotifyFriendOnline(grpc::ServerContext *context,
                                   const NotifyFriendOnlineRequest *request,
                                   NotifyFriendOnlineResponse *response) {
    auto to_uid = request->touid();
    auto session = UserManager::GetInstance()->GetSession(to_uid);
    Defer defer([request, response]() {
        response->set_error(static_cast<int>(ErrorCodes::SUCCESS));
...
Status ChatGrpcServer::NotifyKickUser(grpc::ServerContext *context,
