...
void LogicSystem::RegisterCallBacks() {
    ...
    _function_callbacks[MsgId::ID_ADD_FRIEND_REQ] =
        [this](std::shared_ptr<Session> session, uint16_t msg_id,
            ...
            if (apply_each) {
                ...
                std::string status;
                std::string status_key =
                    USER_STATUS_PREFIX + std::to_string(toUid);
                RedisManager::GetInstance()->Get(status_key, status);
            ...
            std::string status;
            std::string status_key = USER_STATUS_PREFIX + std::to_string(toUid);
            RedisManager::GetInstance()->Get(status_key, status);
            if (!b_ip || status != "1") {
                return;
            }
    ...
    _function_callbacks[MsgId::ID_AUTH_FRIEND_REQ] =
        [this](std::shared_ptr<Session> session, uint16_t msg_id,
            ...
            auto fromEmail = j["from_email"].get<std::string>();
    ...
    _function_callbacks[MsgId::ID_TEXT_CHAT_MSG_REQ] =
        [this](std::shared_ptr<Session> session, uint16_t msg_id,
            ...
            std::string status;
            std::string status_key =
                USER_STATUS_PREFIX + std::to_string(to_uid);
            RedisManager::GetInstance()->Get(status_key, status);
    ...
    _function_callbacks[MsgId::ID_SYNC_STATUS] = [this](
        ...
        Defer defer([this, &j, session]() {
            session->Send(j.dump(),
                          static_cast<int>(MsgId::ID_SYNC_STATUS_RSP));
        });

        auto uid = j["uid"].get<int>();
        auto uid_str = std::to_string(uid);
        auto status = j["status"].get<int>();
        std::string key = USER_STATUS_PREFIX + std::to_string(uid);
        if (RedisManager::GetInstance()->Set(key, std::to_string(status))) {
            j["error"] = ErrorCodes::SUCCESS;
            if (status == 1) {
                json jj;
                // 获取申请列表
                ...
                std::vector<std::shared_ptr<UserInfo>> notification_list;
                bool b_notify =
                    MysqlManager::GetInstance()->GetNotificationList(
                        uid_str, notification_list);
                if (b_notify && notification_list.size() > 0) {
                    json notifications;
                    for (auto &notification : notification_list) {
                        ...
                        item["message"] =
                            notification
                                ->desc; // 用desc代表message借用UserInfo的结构。
                        item["time"] = notification->back; // 备用字段表示时间。
                        notifications.push_back(item);
                    }
                    jj["notifications"] = notifications;
                ...
                if (b_unread && unread_messages.size() > 0) {
                    ...
                    for (auto &message : unread_messages) {
                        ...
                        message_item["timestamp"] = message->timestamp();
                        message_item["env"] = message->env();
                        message_item["content_type"] =
                            message->content().type();
                        message_item["content_data"] =
                ...
                jj["uid"] = uid;
                session->Send(jj.dump(),
                              static_cast<int>(MsgId::ID_SYNC_INFORMATIONS));
            }
        }
    };
}
